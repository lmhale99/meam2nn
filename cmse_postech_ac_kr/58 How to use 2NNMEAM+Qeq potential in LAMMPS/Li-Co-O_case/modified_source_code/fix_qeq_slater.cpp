...
void FixQEqSlater::extract_streitz()
{
  ...
  memory->create(x1,ntypes+1,"qeq:x1");			// @@@
  memory->create(j1,ntypes+1,"qeq:j1");			// @@@
  memory->create(x2,ntypes+1,"qeq:x2");			// @@@
  memory->create(j2,ntypes+1,"qeq:j2");			// @@@
  memory->create(x3,ntypes+1,"qeq:x3");			// @@@
  memory->create(j3,ntypes+1,"qeq:j3");			// @@@
  memory->create(x4,ntypes+1,"qeq:x4");			// @@@
  memory->create(j4,ntypes+1,"qeq:j4");			// @@@
  memory->create(pen,ntypes+1,"qeq:pen");		// @@@
...
//eta = (double *) pair->extract("eta",tmp);
  ...
  x1 = (double *) pair->extract("x1",tmp);		// @@@
  j1 = (double *) pair->extract("j1",tmp);		// @@@
  x2 = (double *) pair->extract("x2",tmp);		// @@@
  j2 = (double *) pair->extract("j2",tmp);		// @@@
  x3 = (double *) pair->extract("x3",tmp);		// @@@
  j3 = (double *) pair->extract("j3",tmp);		// @@@
  x4 = (double *) pair->extract("x4",tmp);		// @@@
  j4 = (double *) pair->extract("j4",tmp);		// @@@
  pen = (double *) pair->extract("pen",tmp);	// @@@

//if (chi == NULL || eta == NULL || gamma == NULL
//                || zeta == NULL || zcore == NULL)
  if ( gamma == NULL || zeta == NULL || zcore == NULL
	|| x1 == NULL || j1 == NULL || x2 == NULL || j2 == NULL
	|| x3 == NULL || j3 == NULL || x4 == NULL || j4 == NULL || pen == NULL )	// @@@
    error->all(FLERR,
...
//  calculate_Q();				// @@@
  ...
  if ( i > maxrepeat && comm->me == 0 ) {										// @@@
    char str[128];																// @@@
    sprintf(str,"Fix qeq some charges not bound within the domain: %d atoms "	// @@@
            "at " BIGINT_FORMAT " step",nout,update->ntimestep);				// @@@
...
void FixQEqSlater::sparse_matvec( sparse_matrix *A, double *x, double *b )
{
  ...
  for( i = 0; i < nlocal; ++i ) {
	...
	{  qi = q[i];															// @@@
	   ip = atom->type[i];													// @@@
	  if ( qi*pen[ip] < 0.0 ) {												// @@@
	    b[i] = (abs(pen[ip]) - 2.0*force->qqr2e*woself) * x[i];				// @@@
	  } else if ( abs(qi) < 1.0 ) {											// @@@
	    b[i] = (j1[ip] - 2.0*force->qqr2e*woself) * x[i]; 					// @@@
	  } else if ( abs(qi) < 2.0 ) {                                  		// @@@
	    b[i] = (j2[ip] - 2.0*force->qqr2e*woself) * x[i];  					// @@@
	  } else if ( abs(qi) < 3.0 ) {                                   		// @@@
	    b[i] = (j3[ip] - 2.0*force->qqr2e*woself) * x[i];  					// @@@
	  } else {                                                        		// @@@
	    b[i] = (j4[ip] - 2.0*force->qqr2e*woself) * x[i];  					// @@@
	  }                                                               		// @@@
	}																		// @@@
//    b[i] = (eta[ip] - 2.0*force->qqr2e*woself) * x[i];
  }
...
int FixQEqSlater::calculate_check_Q()  // @@@ Copied from 'calculate_Q()' in fix_qeq.cpp and modified
{
  int i, k, inum, ii;
  int *ilist;
  ...
  for( ii = 0; ii < inum; ++ii ) {
    ...
    if (atom->mask[i] & groupbit) {
	  ...
	  qi_new = q[i];								// @@@
	  if ( qi_new*qi_old < 0.0 ||					// @@@
		int(abs(qi_new)) != int(abs(qi_old)) ) n++;	// @@@
	}
  }
