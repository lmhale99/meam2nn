...
PairCoulStreitz::~PairCoulStreitz()
{
  ...
  if (allocated) {
    ...
    memory->destroy(qeq_x1);		// @@@
    memory->destroy(qeq_j1);		// @@@
    memory->destroy(qeq_x2);		// @@@
    memory->destroy(qeq_j2);		// @@@
    memory->destroy(qeq_x3);		// @@@
    memory->destroy(qeq_j3);		// @@@
    memory->destroy(qeq_x4);		// @@@
    memory->destroy(qeq_j4);		// @@@
    memory->destroy(qeq_g);
    memory->destroy(qeq_z);
    memory->destroy(qeq_c);
    memory->destroy(qeq_p);			// @@@
    delete [] map;
  }
}
...
void PairCoulStreitz::allocate()
{
 ...
 memory->create(qeq_x1,n+1,"pair:qeq_x1");			// @@@
 memory->create(qeq_j1,n+1,"pair:qeq_j1");			// @@@
 memory->create(qeq_x2,n+1,"pair:qeq_x2");			// @@@
 memory->create(qeq_j2,n+1,"pair:qeq_j2");			// @@@
 memory->create(qeq_x3,n+1,"pair:qeq_x3");			// @@@
 memory->create(qeq_j3,n+1,"pair:qeq_j3");			// @@@
 memory->create(qeq_x4,n+1,"pair:qeq_x4");			// @@@
 memory->create(qeq_j4,n+1,"pair:qeq_j4");			// @@@
 memory->create(qeq_g,n+1,"pair:qeq_g");
 memory->create(qeq_z,n+1,"pair:qeq_z");
 memory->create(qeq_c,n+1,"pair:qeq_c");
 memory->create(qeq_p,n+1,"pair:qeq_p");			// @@@
...
//int params_per_line = 6;
  ...
  while (1) {
    ...
    params[nparams].de2 = atof(words[3]);	// @@@
    params[nparams].de3 = atof(words[4]);	// @@@
    params[nparams].de4 = atof(words[5]);	// @@@
    params[nparams].gamma = atof(words[6]);	// @@@
    params[nparams].zeta = atof(words[7]);	// @@@
    params[nparams].zcore = atof(words[8]);	// @@@
    params[nparams].pen = atof(words[9]);	// @@@

    // parameter sanity check

    if (params[nparams].eta < 0.0 || params[nparams].de2 < 0.0 || params[nparams].de3 < 0.0 || params[nparams].de4 < 0.0 ||
		params[nparams].zeta < 0.0 || params[nparams].zcore < 0.0 || params[nparams].gamma != 0.0 || params[nparams].pen == 0.0 )
      error->all(FLERR,"Illegal coul/streitz parameter");	// @@@

	// set parameter ion, j1-4, x1-4, e1-4 @@@
	double ion = params[nparams].pen/abs(params[nparams].pen);					// @@@

	params[nparams].j1 = params[nparams].eta;									// @@@
	params[nparams].j2 = params[nparams].j1 + 2.0*params[nparams].de2;			// @@@
	params[nparams].j3 = params[nparams].j2 + 2.0*params[nparams].de3;			// @@@
	params[nparams].j4 = params[nparams].j3 + 2.0*params[nparams].de4;			// @@@

	params[nparams].x1 = params[nparams].chi;									// @@@
	params[nparams].x2 = params[nparams].x1 - 2.0*ion*params[nparams].de2;		// @@@
	params[nparams].x3 = params[nparams].x2 - 4.0*ion*params[nparams].de3;		// @@@
	params[nparams].x4 = params[nparams].x3 - 6.0*ion*params[nparams].de4;		// @@@

	params[nparams].e1 = 0.0;													// @@@
	params[nparams].e2 = params[nparams].e1 + 1.0*params[nparams].de2;			// @@@
	params[nparams].e3 = params[nparams].e2 + 4.0*params[nparams].de3;			// @@@
	params[nparams].e4 = params[nparams].e3 + 9.0*params[nparams].de4;			// @@@
...
// double s1=param->chi, s2=param->eta;
 ...
 // Set coefficients of self ionization energy term accordig to charge @@@
 if ( qi*param->pen < 0.0 ) {										// @@@
	 s0=0.0;														// @@@
	 s1=param->x1;													// @@@
	 s2=abs(param->pen);											// @@@
   ...
   if (kspacetype == 1) //return 1.0*qi*(s1+qi*(0.50*s2 - qqrd2e*woself));
	  	return 1.0*qi*(s1+qi*(0.50*s2 - qqrd2e*woself)) + s0;		// @@@

   if (kspacetype == 2) //return 1.0*qi*(s1+qi*(0.50*s2));
   		return 1.0*qi*(s1+qi*(0.50*s2)) + s0;						// @@@
...
/*if (strcmp(str,"chi") == 0 && qeq_x) {
  ...
  if (strcmp(str,"x1") == 0 && qeq_x1) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_x1[i] = params[map[i]].x1;				// @@@
      else qeq_x1[i] = 0.0;											// @@@
    return (void *) qeq_x1;											// @@@
  }																	// @@@
  if (strcmp(str,"j1") == 0 && qeq_j1) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_j1[i] = params[map[i]].j1;				// @@@
      else qeq_j1[i] = 0.0;											// @@@
    return (void *) qeq_j1;											// @@@
  }																	// @@@
  if (strcmp(str,"x2") == 0 && qeq_x2) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_x2[i] = params[map[i]].x2;				// @@@
      else qeq_x2[i] = 0.0;											// @@@
    return (void *) qeq_x2;											// @@@
  }																	// @@@
  if (strcmp(str,"j2") == 0 && qeq_j2) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_j2[i] = params[map[i]].j2;				// @@@
      else qeq_j2[i] = 0.0;											// @@@
    return (void *) qeq_j2;											// @@@
  }																	// @@@
  if (strcmp(str,"x3") == 0 && qeq_x3) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_x3[i] = params[map[i]].x3;				// @@@
      else qeq_x3[i] = 0.0;											// @@@
    return (void *) qeq_x3;											// @@@
  }																	// @@@
  if (strcmp(str,"j3") == 0 && qeq_j3) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_j3[i] = params[map[i]].j3;				// @@@
      else qeq_j3[i] = 0.0;											// @@@
    return (void *) qeq_j3;											// @@@
  }																	// @@@
  if (strcmp(str,"x4") == 0 && qeq_x4) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_x4[i] = params[map[i]].x4;				// @@@
      else qeq_x4[i] = 0.0;											// @@@
    return (void *) qeq_x4;											// @@@
  }																	// @@@
  if (strcmp(str,"j4") == 0 && qeq_j4) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_j4[i] = params[map[i]].j4;				// @@@
      else qeq_j4[i] = 0.0;											// @@@
    return (void *) qeq_j4;											// @@@
  }																	// @@@
  if (strcmp(str,"pen") == 0 && qeq_p) {							// @@@
    dim = 1;														// @@@
    for (int i = 1; i <= atom->ntypes; i++)							// @@@
      if (map[i] >= 0) qeq_p[i] = params[map[i]].pen;				// @@@
      else qeq_p[i] = 0.0;											// @@@
    return (void *) qeq_p;											// @@@
  }																	// @@@
